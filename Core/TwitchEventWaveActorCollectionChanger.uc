//TwitchEventWaveActor that modifies the monster collection (set of zeds used during regular wave spawning) for the given waves this wave actor is relevant for. Will revert changes on completion.
...
function UpdateMonsterCollection(KFGameType GameType, class<KFMonstersCollection> Collection, array<KFMonstersCollection.MClassTypes> MonsterClasses)
{
	...
	//If we want to see exactly what the new squads are in human readable form.
	//DebugSquads();
}
