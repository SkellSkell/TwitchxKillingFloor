class TE_MicroFPBomb extends TwitchEventSpawner;

static function string GetSpawnerResponse(Pawn Player, string Instigator, int NumSpawned)
{
	return "Look out! "$Instigator$" has called in "$string(NumSpawned)$" Micro Fleshpounds!";
}
...
{
	local int Count;
	local ZombieFleshpound ZombieFleshpound;

	Count = 0;
	foreach KFGameType.DynamicActors(class'ZombieFleshpound', ZombieFleshpound, 'Twitch')
	{
		Count++;
	}
...
static function ProcessSpawnedZed(KFMonster SpawnedZed, TwitchEventMut TwitchEventMut)
{
	...
	PlayerEffect = TwitchEventMut.Spawn(class'PE_MicroFleshpound');
...
defaultproperties
{
	...
	ZedList[0] = ZT_Fleshpound;
	ZedList[1] = ZT_Fleshpound;
	ZedList[2] = ZT_Fleshpound;
	ZedList[3] = ZT_Fleshpound;
}